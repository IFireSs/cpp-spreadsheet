...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        auto ref_value = cell->GetValue();
        if (std::holds_alternative<double>(ref_value)) {
            return std::get<double>(ref_value);
        }
        if (std::holds_alternative<std::string>(ref_value)) {
            auto value = std::get<std::string>(ref_value);
            double result = 0;
            if (!value.empty()) {
        ...
        throw std::get<FormulaError>(ref_value);
    }
